...
class LedgerScreen extends StatefulWidget {
  ...
  const LedgerScreen({super.key, required this.timeStr});
...
class _LedgerScreenState extends State<LedgerScreen> {
  ...
  Future<void> _loadAndDecryptBalFile() async {
    try {
      ByteData data = await rootBundle.load("assets/web/ledger.bal");
      ...
      setState(() {
        _decryptedHtml = "<html><body><h2>Ledger Load Error</h2><p>${e.toString()}</p></body></html>";
        _isLoading = false;
      });
    }
  }
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      appBar: AppBar(
        title: const Text("စာရင်းချုပ်မျက်နှာပြင်"),
            ...
            InAppWebView(
              ...
              onLoadStop: (controller, url) async {
                await controller.evaluateJavascript(source: """
                  if (typeof initLedgerData === 'function') {
                    initLedgerData('$_selectedDate', '$_selectedTime');
                  }
                """);
              },
            ),
          if (_isLoading)
            const Center(
              child: Column(
                ...
                children: [
                  CircularProgressIndicator(valueColor: AlwaysStoppedAnimation<Color>(Color(0xFF2C5282))),
                  SizedBox(height: 15),
                  Text("Loading Ledger Interface...", style: TextStyle(color: Colors.grey)),
                ],
              ),
            ),
        ],
